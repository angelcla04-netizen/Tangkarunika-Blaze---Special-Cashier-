...
function addProductByBarcode(barcode) {
  ...
  if (!item) { flashWarning("Product not found: " + barcode); return; }
  if (!cart[barcode]) { cart[barcode] = { name: item.name, price: item.price, quantity: 0 }; }
...
function handlePay() {
  ...
  if (isNaN(cash) || cash < total) { document.getElementById("changeDisplay").textContent = fmt(0); flashWarning("Cash is less than total."); return; }
...
function saveReceiptHistory() {
  ...
  const record = { id: Date.now(), time: nowIso(), items: cart, total: cartTotal(),
